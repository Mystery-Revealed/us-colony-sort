...
// → approve names → watch live status and class accuracy → download the PDF →
// End Session (deletes everything from memory). Colony Sort is a solo
// knowledge sprint with no pick — one class-wide group — so students start on
// their own as soon as they're approved, no pairing needed.
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Colony Sort: Region Rush · 30 cards, 3 rounds · TEKS 8.2B, 8.12A, 8.29B</p>
  ...
  const classAcc = roster?.classAccuracy?.class || { count: 0, average: 0 };
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Colony Sort: Region Rush · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student begins sorting right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="class-summary single">
          <div className="summary-stat">
            <div className="big-number">{classAcc.count ? `${classAcc.average}%` : '—'}</div>
            <div className="muted">{classAcc.count} completed</div>
          </div>
        </div>
      ...
      <footer className="app-footer">8th Grade U.S. History · Colony Sort: Region Rush · TEKS 8.2B, 8.12A, 8.29B</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [27, 42, 74]; // federal navy (Union Blue §2)

  doc.setFontSize(16);
  doc.text('Colony Sort: Region Rush — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    head: [['Class accuracy', '']],
    body: [
      ['Completed', String(classAcc.count)],
      ['Average accuracy', classAcc.count ? `${classAcc.average}%` : '—'],
    ],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('8th Grade U.S. History · Colony Sort: Region Rush · TEKS 8.2B, 8.12A, 8.29B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`colony-sort_${roster.joinCode}_${date}.pdf`);
}
